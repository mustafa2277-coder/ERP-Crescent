    ...
    $('#defaultModal').on('hidden.bs.modal', function () {
       ...
       function exportTableToCSV($table, filename) {
        ...
        divPanel.each(function () {
            ...
            accname = $(this).find('span#head').text();
            var tbody = $(this).find('table tbody');
 
            tbody.find('tr').each(function () {
            //console.log($(this).find('td').eq(1).text());
            accHeads.date = $(this).find('td').eq(0).text();
            accHeads.entryNum = $(this).find('td').eq(1).text();
            accHeads.project = $(this).find('td').eq(2).text();
            accHeads.debit = $(this).find('td').eq(3).text();
            accHeads.credit = $(this).find('td').eq(4).text();
            accHeads.balance = $(this).find('td').eq(5).text();
          //  console.log(item.eq(1).text());
           ...
           transactons[accname]={data:inner};
