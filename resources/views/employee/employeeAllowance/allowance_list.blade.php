            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="margin-top: 20px;">
                    <div class="card">
                        <div class="header">
                            <h2>
                                Employees Allowance List
                                <a class="btn btn-primary btn-circle waves-effect waves-circle waves-float" style="margin-bottom: 14px;float:right;" id="add_new" accesskey="+" href="{{ url('/getAddEmployeeAllowance')}}"> 
                        ...
                        <?php   $i=1;  ?>
                            <table class="table table-striped table-hover js-exportable">
                                ...
                                <tbody>
                                @foreach($employeesAllowance as $employeeAllowance)
                                    <tr>
                                        <th scope="row">{{$i++}}</th>
                                        <td>{{$employeeAllowance->name}}</td>
                                        <td>{{$employeeAllowance->alwnc}}</td>
                                        <td>{{$employeeAllowance->amount}}</td>
                                        <td>
                                            <a href="{{ url('/getEditEmployeeAllowance') }}/{{$employeeAllowance->id}}" ><i class="material-icons">edit</i></a>
                                        </td>
                                    </tr>
                                @endforeach  
                                </tbody>
